# -*- coding: utf-8 -*-
"""
Created on Wed Apr 21 09:20:10 2021
...
node_file_name = './data/邓小平时代-人物节点-分类.csv' # 手工增加国别分类
link_file_name = './data/邓小平时代-人物连接.csv'
# 输出文件
out_file_name = './output/关系图-分类-邓小平时代.html'
...
for one_line in node_line_list:
    ...
    size_temp = int(one_line_list[1])/20 # 手动调整节点的尺寸
    if one_line_list[0] == "邓小平":
        size_temp = size_temp/3
    #主要人物邓小平权重过大，为美化视图适量缩小
    node_in_graph.append(opts.GraphNode(
            name=one_line_list[0], 
            value=int(one_line_list[1]), 
            symbol_size=size_temp, 
...
c.set_global_opts(title_opts=opts.TitleOpts(title="关系图-邓小平时代-分类"))
